...
class ImportData:
    def __init__(self, file_path):
        ...
        for child in root:
            ...
            if child.tag == "P_T":
                self.data[child.find("P_ID").text] = dict(
                    ...
                    Date=QDateTime.fromString(
                        self.get_value(child, "P_Date"), "dd.MM.yyyy hh:mm:ss"
                    ),
                    Channel="",
                    Sections={},
                )
            ...
            # section
            if child.tag == "S_T":
                self.data[child.find("S_Project_ID").text]["Sections"][child.find("S_ID").text] = (
                    dict(
                ...
                for p_id, project in self.data.items():
                    for s_id in project["Sections"]:
                        if s_id == child.find("SI_Section_ID").text:
                            self.data[p_id]["Sections"][s_id]["Inspections"][
                                child.find("SI_ID").text
                            ] = dict(
                                ...
                                InspectedLength=float(
                                    self.get_value(child, "SI_InspectedLength") or 0
                                ),
                                ...
                                InspDate=QDate.fromString(
                                    self.get_value(child, "SI_InspDate"), "dd.MM.yyyy"
                                ),
                    ...
                    for s_id, section in project["Sections"].items():
                        for i_id in section["Inspections"]:
                            if i_id == child.find("SO_Inspecs_ID").text:
                                ...
                                self.data[p_id]["Sections"][s_id]["Inspections"][i_id][
                                    "Observations"
                                ][child.find("SO_ID").text] = dict(
                                    ...
                                    Import=True
                                    if code is not None and code not in SkipCode
                                    else False,
        ...
        for p_id in self.data.keys():
            ...
            self.data[p_id]["Sections"] = dict(
                sorted(self.data[p_id]["Sections"].items(), key=lambda t: int(t[1]["Counter"]))
            )

            for s_id in self.data[p_id]["Sections"].keys():
                for i_id in self.data[p_id]["Sections"][s_id]["Inspections"].keys():
                    self.data[p_id]["Sections"][s_id]["Inspections"][i_id]["Observations"] = dict(
                        sorted(
                            self.data[p_id]["Sections"][s_id]["Inspections"][i_id][
                                "Observations"
                            ].items(),
